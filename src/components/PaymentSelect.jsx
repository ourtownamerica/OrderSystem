...
export default function PaymentSelect({onChange, initialValue = null, disabled=false}){
	...
	const setSelectRef = useCallback(node => {
		...
		if (!$(node).data('select2')) {
			$(node).select2({
				...
				ajax: {
					url: 'https://rockwell.ourtownamerica.com/intra/api/ordersys/select2cards.php',
					dataType: 'json',
					delay: 250,
					data: function(p){
						return {
			...
			$(node).on('select2:select', function (e) {
				...
				if (selectedData && selectedData.length > 0) {
					...
					setErrorText('No Payment Method selected.');
					onChange(null);
				}
			});
